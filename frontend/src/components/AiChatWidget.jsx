...
import { Send, Sparkles, AlertTriangle, ShieldCheck, CheckCircle2, HelpCircle, XCircle, Info } from 'lucide-react';
...
export default function AiChatWidget({ productId, productName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await askAiAdvisor(userMessage, i18n.language);
    ...
    <div className="card flex flex-col h-[600px] lg:h-[700px] border-t-4 border-t-[#d99c9c] shadow-xl relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6 bg-[#fdfaf6] z-10 relative">
        {messages.map((msg, idx) => {
          ...
          const cardBorder = isDanger ? 'border-red-400 bg-red-50' : 'border-premium-100 bg-white';
          ...
          return (
            <div key={idx} className="flex justify-start">
              <div className={`max-w-[95%] border ${cardBorder} shadow-sm rounded-3xl rounded-tl-sm p-5 flex flex-col gap-4`}>
                ...
                {/* Main Text */}
                <p className={`text-sm leading-relaxed ${isDanger ? 'text-red-900 font-medium' : 'text-premium-800'}`}>
                  {getAiMessageText(msg)}
                </p>

                {/* Structured Data Badges (only if not greeting/error) */}
                {!msg.type && (
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white border border-premium-100 shadow-sm rounded-3xl rounded-tl-sm p-5 flex gap-2">
      ...
      <form onSubmit={handleSubmit} className="p-4 bg-white border-t border-premium-100 z-10 relative">
        <div className="relative">
          <input
            ...
            className="input-field rtl:pl-24 ltr:pr-24 text-sm font-medium"
            disabled={isLoading}
          />
          <button 
            type="submit" 
            disabled={isLoading || !input.trim()}
            className="absolute rtl:left-2 ltr:right-2 top-1/2 -translate-y-1/2 px-5 py-2.5 bg-[#d99c9c] text-white rounded-xl font-medium text-sm hover:bg-[#c68989] disabled:opacity-50 transition-colors shadow-sm"
          >
            Ask AI
          </button>
        </div>
